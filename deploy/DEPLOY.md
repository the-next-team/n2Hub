...
ssh -i "$env:USERPROFILE\.ssh\n2hub_deploy" n2soft@211.191.65.14 "chmod -R a+rX ~/n2hub/dist"
```

> 컨테이너 재시작 불필요 — 정적 파일이라 업로드 즉시 반영 (브라우저 새로고침만)
...
- **https로 바꾸지 말 것**: OnlyOffice가 http(8090)라서 n2Hub를 https로 서빙하면 mixed content 차단됨
